...
public class LoyaltyControl {
    ...
    // ADT Lists
    //==================================================
    private DoublyLinkedList<LoyaltyRecord> loyaltyList = new DoublyLinkedList<>();
    private DoublyLinkedList<RedemptionRecord> redemptionList = new DoublyLinkedList<>();
    ...
    public LoyaltyControl() {
        ...
        DoublyLinkedList<LoyaltyRecord> loadedList = loyaltyDatabase.retrieveFromFile();

        if (loadedList != null) {
            loyaltyList = loadedList;
        }
    ...
    private void refreshLoyaltyData() {
        ...
        DoublyLinkedList<LoyaltyRecord> latestList = loyaltyDatabase.retrieveFromFile();
    ...
    public DoublyLinkedList<LoyaltyRecord> getGuestsByTier(String tier) {
        ...
        DoublyLinkedList<LoyaltyRecord> result = new DoublyLinkedList<>();
    ...
    //==================================================
    public boolean createLoyaltyMember(Guest guest, int lifetimePoints) {

        if (guest == null || guest.getGuestID() == null || guest.getGuestID().trim().isEmpty()) {
        ...
        LoyaltyRecord existingRecord = searchGuest(guest.getGuestID());
        ...
        if (existingRecord != null) {
            ...
            existingRecord.setLifetimePoints(lifetimePoints);

            updateTier(existingRecord);

            existingRecord.setExpiryDate(LocalDate.now().plusYears(1));

            saveLoyaltyData();

            lastMessage = "Loyalty member updated successfully.";
        ...
        // New Loyalty Member
        //==================================================
        LoyaltyRecord newRecord = new LoyaltyRecord(
                guest,
                0,
                lifetimePoints,
        ...
        lastMessage = "Loyalty member created successfully.";
    ...
    public int getRewardPoints(String rewardName) {
        ...
        if (rewardName.equalsIgnoreCase(WELCOME_DRINK)) {
            return 200;
        }

        if (rewardName.equalsIgnoreCase(FOOD_VOUCHER)) {
            return 300;
        }

        if (rewardName.equalsIgnoreCase(FREE_MEAL)) {
            return 500;
        }

        if (rewardName.equalsIgnoreCase(LATE_CHECK_OUT)) {
            return 700;
        }

        if (rewardName.equalsIgnoreCase(ROOM_UPGRADE)) {
            return 1000;
        }

        if (rewardName.equalsIgnoreCase(SPA_VOUCHER)) {
            return 1200;
        }

        if (rewardName.equalsIgnoreCase(LOUNGE_ACCESS)) {
            return 1500;
        }

        if (rewardName.equalsIgnoreCase(FREE_NIGHT_STAY)) {
            return 2500;
        }
    ...
    public boolean canTierRedeemReward(String tier, String rewardName) {
        ...
        for (String reward : rewards) {
            ...
            if (reward.equalsIgnoreCase(rewardName)) {
                return true;
            }
        }
    ...
    //==================================================
    public DoublyLinkedList<LoyaltyRecord> getPointsExpiringWithin30Days() {
        ...
        for (int i = 1; i <= loyaltyList.getSize(); i++) {
            ...
            if (notExpired && within30Days && hasPoints) {
                result.add(record);
            }
        }
    ...
    //==================================================
    public DoublyLinkedList<LoyaltyRecord> getExpiredPointsGuests() {
    ...
    public int processExpiredPoints() {
        ...
        DoublyLinkedList<LoyaltyRecord> expiredList = getExpiredPointsGuests();

        int totalProcessed = expiredList.getSize();

        for (int i = 1; i <= expiredList.getSize(); i++) {
    ...
    //==================================================
    public DoublyLinkedList<RedemptionRecord> getTodayRedemptions(String rewardName) {

        DoublyLinkedList<RedemptionRecord> result = new DoublyLinkedList<>();

        LocalDate today = LocalDate.now();

        for (int i = 1; i <= redemptionList.getSize(); i++) {

            RedemptionRecord redemption = redemptionList.getEntry(i);

            if (redemption == null || redemption.getRedemptionDate() == null) {
                continue;
            }
    ...
    public int countTodayRedemptions(String tier, String rewardName) {
        ...
        for (int i = 1; i <= redemptionList.getSize(); i++) {
            ...
            if (sameDate && sameTier && sameReward) {
                count++;
            }
        }
    ...
    public int calculateTodayTotalPointsUsed() {
        ...
        for (int i = 1; i <= redemptionList.getSize(); i++) {
            ...
            if (redemption.getRedemptionDate().equals(today)) {
                ...
                int points = getRewardPoints(redemption.getRewardName());
    ...
    //==================================================
    public DoublyLinkedList<LoyaltyRecord> getTopFiveMembersByPointsUsed(String selectedTier) {

        refreshLoyaltyData();

        DoublyLinkedList<LoyaltyRecord> filteredList = new DoublyLinkedList<>();
        DoublyLinkedList<LoyaltyRecord> topFiveList = new DoublyLinkedList<>();
        ...
        for (int i = 1; i <= loyaltyList.getSize(); i++) {
            ...
            if (selectedTier.equalsIgnoreCase("All")
                    || record.getLoyaltyTier().equalsIgnoreCase(selectedTier)) {

                filteredList.add(record);
            }
        }

        while (!filteredList.isEmpty() && topFiveList.getSize() < 5) {

            int highestPosition = 1;

            LoyaltyRecord highestRecord = filteredList.getEntry(1);

            int highestPointsUsed = highestRecord.getLifetimePoints() - highestRecord.getAvailablePoints();

            for (int i = 2; i <= filteredList.getSize(); i++) {

                LoyaltyRecord currentRecord = filteredList.getEntry(i);

                int currentPointsUsed = currentRecord.getLifetimePoints() - currentRecord.getAvailablePoints();

                if (currentPointsUsed > highestPointsUsed) {

                    highestPointsUsed = currentPointsUsed;
                    highestRecord = currentRecord;
                    highestPosition = i;
                }
            }

            topFiveList.add(highestRecord);

            filteredList.remove(highestPosition);
        }
    ...
    // Getter Methods
    //==================================================
    public DoublyLinkedList<LoyaltyRecord> getAllLoyaltyRecords() {
    ...
    public DoublyLinkedList<RedemptionRecord> getAllRedemptions() {
    ...
    // Setter Methods
    //==================================================
    public void setLoyaltyList(DoublyLinkedList<LoyaltyRecord> loyaltyList) {
