...
public class LoyaltyAndRewardsUI {
    ...
    private void displayLoyaltyMembers() {
        ...
        InputUtility.clearScreen();
        System.out.printf("Selected Tier: %s", tier);
    ...
    private void displayLoyaltyResult(
        ...
        for (int i = 1; i <= result.getNumberOfEntries(); i++) {
            ...
            System.out.printf(
                    ...
                    i,
                    limitText(String.valueOf(record.getGuestID()), 10),
                    limitText(String.valueOf(record.getGuestName()), 23),
                    limitText(String.valueOf(record.getLoyaltyTier()), 12),
                    record.getAvailablePoints(),
                    record.getLifetimePoints(),
                    limitText(String.valueOf(record.getExpiryDate()), 12)
            );
        }
        ...
        // Total members
        String totalText = "Total Members: " + result.getNumberOfEntries();
    ...
    private void displayExpiringPointsTable(
        ...
        for (int i = 1; i <= result.getNumberOfEntries(); i++) {
            ...
            System.out.printf(
                    ...
                    limitText(record.getLoyaltyTier(), 12),
                    record.getAvailablePoints(),
                    limitText(String.valueOf(record.getExpiryDate()), 12),
                    loyaltyControl.getDaysRemaining(record)
            );
        ...
        System.out.printf(
                "| %-" + (insideWidth - 2) + "s |%n",
                "Total Members: " + result.getNumberOfEntries()
        );
    ...
    private void displayExpiredPointsTable(
        ...
        System.out.printf(
                "| %-" + (insideWidth - 2) + "s |%n",
                "Total Expired Members: " + result.getNumberOfEntries()
        );
    ...
    private void displaySingleRewardReport(String rewardName) {
        ...
        System.out.printf(
                "| %-20s : %-32s |%n",
                "Reward",
                limitText(rewardName, 32)
        );

        System.out.printf(
                "| %-20s : %-32s |%n",
                "Points Each",
                String.format("%,d", pointsEach)
        );
    ...
    private void displayRewardTierMatrix() {
        ...
        for (String tier : tiers) {
            ...
            for (String reward : rewards) {
                int count
                        = loyaltyControl.countTodayRedemptions(
                                tier,
                                reward
                        );
        ...
        for (String reward : rewards) {
            int rewardTotal
                    = loyaltyControl.countTodayRedemptionsByReward(
                            reward
                    );
        ...
        System.out.printf(
                ...
                String.format(
                        "%,d",
                        loyaltyControl.calculateTodayTotalPointsUsed()
                )
        );
    ...
    // ==================================================
        ...
        DoublyLinkedList.ArrayList<LoyaltyRecord> result
                = loyaltyControl.getTopFiveMembersByPointsUsed(tier);
        ...
        for (int i = 1; i <= result.getNumberOfEntries(); i++) {
            ...
            System.out.printf(
                    ...
                    limitText(record.getGuestName(), 23),
                    limitText(record.getLoyaltyTier(), 12),
                    String.format("%,d", record.getLifetimePoints()),
                    String.format("%,d", pointsUsed)
            );
        }
        ...
        System.out.printf(
                ...
                String.format("%,d", totalPointsUsed)
        );
